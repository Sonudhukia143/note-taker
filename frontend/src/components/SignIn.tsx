...
export default function SignIn({ toggleSignUp }: ToggleSignUp) {
        ...
        <>
            <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
                <div className="sm:mx-auto sm:w-full sm:max-w-sm">
                    <h2 className="mt-2 text-center text-2xl/9 font-bold tracking-tight text-black">Sign In to your account</h2>
                </div>
                ...
                <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-sm">
                    <form onSubmit={(e) => signIn(e)} className="space-y-6">
                        <div>
                            ...
                            <div className="mt-2">
                                <input {...isOtpSent && { readOnly: true }} id="username" onChange={(e) => { setFormData({ ...formData, username: e.target.value }) }} type="username" name="username" required autoComplete="username" className="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-black outline-1 outline-offset-1 outline-indigo/100 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
                            </div>
                        </div>
...

                    {
                        ...
                        <>
                            ...
                            <button onClick={(e) => handleSubmit(e)} className="m-2 flex mx-auto justify-center rounded-md bg-indigo-500 px-3 py-1.5 text-sm/6 font-semibold text-white hover:bg-indigo-400 cursor-pointer focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">{loading?"Submitting":"Submit"}</button>
                        </>
                    }
                    ...
                    <p className="mt-2 text-center text-sm/6 text-gray-400">
                        Not a member?
                        <button onClick={(e) => toggleSignUp(e)} className="font-semibold text-indigo-400 hover:text-indigo-300 cursor-pointer m-2"> Sign Up</button>
                    </p>
                </div>
            </div>
